...
class MediaRecommendationEngine {
  static List<MediaRecommendation> forMedia(
    ...
    for (final candidate in library) {
      ...
      if (sharedActors.isNotEmpty) {
        ...
        reasons.add(
          'Because you watched ${source.title}, you saw $actorNames',
        );
      }
      ...
      final semanticReasons = _semanticReasons(source, candidate);
  ...
  static List<String> _semanticReasons(MediaItem source, MediaItem candidate) {
    ...
    final sharedFamilies = sourceGenres.intersection(candidateGenres);
    ...
    final sharedSubgenres = sourceSubgenres.intersection(candidateSubgenres);
    for (final subgenre in sharedSubgenres.take(3)) {
      reasons.add('same $subgenre subgenre');
    }

    final hybrid = _hybridReason(sourceText, candidateText);
    if (hybrid != null) reasons.add(hybrid);
    ...
    if (source.title.toLowerCase().contains('friends') &&
      ...
      reasons.add('friend-group sitcom with character-driven relationships');
    }
    if (source.title.toLowerCase().contains('friends') &&
        candidate.title.toLowerCase().contains('brooklyn nine-nine')) {
      reasons.add('sitcom with a workplace police setting');
    }
    if (source.title.toLowerCase().contains('friends') &&
        candidate.title.toLowerCase().contains('modern family')) {
      reasons.add('sitcom centered on family relationships');
    }
    if (source.title.toLowerCase().contains('friends') &&
        candidate.title.toLowerCase().contains('abbott elementary')) {
      reasons.add('sitcom centered on a school community');
    }

    if (_isAdultAnimation(source) && _isAdultAnimation(candidate)) {
      reasons.add('similar adult-animation comedy');
    }
  ...
  static String _mediaText(MediaItem media) =>
      ...
      '${media.genres.join(' ')} ${media.tags.join(' ')}'.toLowerCase();

  static Set<String> _genreFamilies(String text) {
    const families = <String, List<String>>{
      'action': ['action', 'adventure', 'martial arts', 'spy', 'disaster'],
      'comedy': ['comedy', 'sitcom', 'romantic comedy', 'rom-com', 'slapstick', 'mockumentary', 'satire'],
      'crime': ['crime', 'gangster', 'heist', 'film noir', 'neo-noir'],
      'drama': ['drama', 'biopic', 'historical drama', 'legal drama', 'courtroom', 'melodrama', 'medical drama', 'teen drama', 'dramedy'],
      'fantasy': ['fantasy', 'high fantasy', 'epic fantasy', 'urban fantasy', 'dark fantasy'],
      'horror': ['horror', 'slasher', 'supernatural horror', 'psychological horror', 'zombie', 'found footage'],
      'mystery': ['mystery', 'whodunit'],
      'romance': ['romance', 'romantic comedy', 'rom-com', 'romantic drama'],
      'science fiction': ['science fiction', 'sci-fi', 'space opera', 'dystopian', 'cyberpunk', 'time travel', 'space western'],
      'sports': ['sports', 'sports drama', 'sports comedy'],
      'thriller': ['thriller', 'psychological thriller', 'crime thriller', 'political thriller', 'techno-thriller'],
      'war': ['war', 'military drama', 'anti-war', 'war biographical'],
      'western': ['western', 'spaghetti western', 'revisionist western', 'epic western'],
      'animation': ['animation', 'animated', 'anime', 'adult animation', 'cgi animation', 'stop motion'],
      'documentary': ['documentary', 'docuseries', 'non-fiction', 'nonfiction'],
    };
    return {
  ...
  static Set<String> _subgenreFamilies(String text) {
    ...
    return {for (final value in subgenres) if (text.contains(value)) value};
  }

  static String? _hybridReason(String sourceText, String candidateText) {
    ...
    for (final pair in pairs) {
      final sourceHas = _containsAny(sourceText, pair);
      final candidateHas = _containsAny(candidateText, pair);
  ...
  static bool _isAdultAnimation(MediaItem media) {
    final text =
        '${media.title} ${media.description ?? ''} '
        '${media.genres.join(' ')} ${media.tags.join(' ')}'
            .toLowerCase();
    ...
    final isAnimation = _containsAny(text, const [
      'animation',
      'animated',
      'cartoon',
    ]);
    final isAdult = _containsAny(text, const [
      'adult',
      'mature',
      'satire',
      'adult comedy',
    ...
    // These well-known adult-animation titles are only classification hints;
...
class SeasonalCollectionEngine {
  ...
  static List<MediaItem> matching(
    ...
    final terms = _termsForMonth((date ?? DateTime.now()).month);
  ...
  /// Songs are dynamically included in the current seasonal collection without
  /// copying or moving the underlying audio file.
  static List<MusicTrack> matchingSongs(
    ...
    final matches = tracks.where((track) {
      final text =
          '${track.title} ${track.artist} ${track.album} ${track.genres.join(' ')} ${track.subgenres.join(' ')}'
              .toLowerCase();
      return terms.any(text.contains);
    }).toList();
    return matches;
      ...
      body: ListView(
        ...
        children: [
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: UniversalText(
                      'This is a dynamic seasonal collection. Matching movies, TV episodes/shows and songs are automatically included when they exist in your account library.',
          ...
          if (matches.isEmpty && songs.isEmpty)
            const Card(
              child: Padding(
                ...
                child: Column(
                  children: [
                    Icon(Icons.auto_awesome_outlined, size: 42, color: Colors.white38),
                    SizedBox(height: 10),
                    UniversalText('No matching media yet', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900)),
                    SizedBox(height: 5),
                    UniversalText(
                      'Matching movies, TV episodes/shows and songs will appear automatically when they are imported into your library.',
          ...
          if (matches.isNotEmpty) ...[
            const UniversalText('MOVIES & TV', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1.3, color: Colors.white54)),
            ...
            for (final media in matches)
              Card(
                child: ListTile(
                  ...
                  subtitle: Text([media.type, if (media.releaseYear != null) '${media.releaseYear}', ...media.genres.take(2)].join(' • ')),
          ...
          if (songs.isNotEmpty) ...[
            ...
            const UniversalText('SONGS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1.3, color: Colors.white54)),
            const SizedBox(height: 8),
            for (final track in songs)
              Card(
                child: ListTile(
                  leading: track.artworkUrl == null ? const Icon(Icons.music_note_rounded) : ClipRRect(borderRadius: BorderRadius.circular(7), child: Image.network(track.artworkUrl!, width: 48, height: 48, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.music_note_rounded))),
