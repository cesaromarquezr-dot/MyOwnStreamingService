...
class _HomeWidgetsScreenState extends State<HomeWidgetsScreen> { final widgets = <String>['Continue Watching','Wishlist','Watch Streak','Recommendations','Recently Added','Storage']; @override Widget build(BuildContext context) => Scaffold(appBar: AppBar(title: const Text('Home Widgets')), body: ReorderableListView.builder(padding: const EdgeInsets.all(16), itemCount: widgets.length, onReorderItem: (oldIndex, newIndex) => setState(() { final item = widgets.removeAt(oldIndex); widgets.insert(newIndex, item); }), itemBuilder: (_, i) => KeyedSubtree(key: ValueKey('home-widget-$i-${widgets[i]}'), child: Card(child: ListTile(tileColor: Colors.transparent, leading: const Icon(Icons.drag_handle_rounded), title: Text(widgets[i]), trailing: const Icon(Icons.widgets_rounded)))))); }
