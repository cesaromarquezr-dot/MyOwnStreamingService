...
-- Seller-controlled checkout policy. Commission is deducted from seller
-- proceeds; it is not added as a hidden card charge to the buyer.
alter table public.shop_stores
  add column if not exists enabled_payment_methods text[] not null default array['subscriptionCard','newCard','applePay','googlePay','paypal','shopPay'],
  add column if not exists platform_commission_percent numeric(5,2) not null default 5 check (platform_commission_percent >= 5),
  add column if not exists default_shipping_cost numeric(12,2) not null default 0 check (default_shipping_cost >= 0),
  add column if not exists payout_currency text not null default 'USD';

alter table public.shop_orders
  add column if not exists buyer_display_currency text not null default 'USD',
  add column if not exists platform_fee numeric(12,2) not null default 0 check (platform_fee >= 0),
  add column if not exists seller_payout numeric(12,2) not null default 0 check (seller_payout >= 0),
...
create index if not exists idx_shop_order_notifications_store on public.shop_order_notifications(store_id, created_at desc);
create index if not exists idx_shop_order_messages_order on public.shop_order_messages(order_id, created_at);
...
-- Seller promotions. A promotion belongs to one store and is evaluated at
-- checkout without changing the catalog price shown on the product.
create table if not exists public.shop_promotions (
  id uuid primary key default gen_random_uuid(),
  store_id uuid not null references public.shop_stores(id) on delete cascade,
  promotion_type text not null check (promotion_type in ('buy_x_get_y','coupon_percent')),
  is_active boolean not null default true,
...
create index if not exists idx_shop_promotions_store_active on public.shop_promotions(store_id, is_active);
create unique index if not exists idx_shop_promotions_store_code
  on public.shop_promotions(store_id, lower(code))
